...
export default function JobTuple({
  ...
  const handleCardClick = () => {
    const url = buildJobDetailsSeoPath({
      title,
      cityName: cityName || '',
  ...
  return (
    <div className="block">
      <div
        className="bg-white rounded-lg shadow p-4 md:p-6 hover:shadow-md transition duration-200 flex flex-col justify-between h-full cursor-pointer"
        role="link"
        tabIndex={0}
      ...
      >
        ...
        <div>
          ...
          {companyId ? (
            <Link
              href={buildCompanySeoPath({ name: company, id: companyId })}
              className="text-blue-900 font-medium text-sm md:text-base mb-2 hover:underline nprogress-trigger inline-block"
              onClick={(e) => e.stopPropagation()}
            >
              {company}
            </Link>
          ) : (
            <div className="text-blue-900 font-medium text-sm md:text-base mb-2">{company}</div>
          )}
        </div>
        {/* Meta: Experience & Location */}
        <div className="flex flex-col sm:flex-row sm:items-center text-gray-600 mb-2 sm:gap-6 gap-1">
          ...
          <div className="flex items-center">
            <svg className="w-4 h-4 md:w-5 md:h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
